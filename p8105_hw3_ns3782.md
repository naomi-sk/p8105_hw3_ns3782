...
Key variables include ride_id (character), a unique identifier for each
ride; rideable_type (character), indicating the type of bike used;
weekdays (character), showing the day of the week; duration (numeric),
representing the length of the ride in minutes; start_station_name and
end_station_name (both character), representing the names of the
starting and ending stations respectively; member_casual (character),
indicating the rider’s membership status; and year and month (both
numeric), indicating when the ride took place.
